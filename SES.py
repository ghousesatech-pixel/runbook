...
DEFAULT_THRESHOLD = float(os.getenv("DEFAULT_THRESHOLD", "0"))
...
def sqs_enrichment(cw, queue_name, alarm_time):
    ...
    ts_age, v_age = get_metric_data_all(
        cw, "AWS/SQS", "ApproximateAgeOfOldestMessage", dims,
        "Average", BASELINE_PERIOD_SECONDS, start_cur, end_cur
    )
    ...
    _, v_sent = get_metric_data_all(
        cw, "AWS/SQS", "NumberOfMessagesSent", dims,
        "Sum", BASELINE_PERIOD_SECONDS, start_rate, end_rate
    )
    _, v_del = get_metric_data_all(
        cw, "AWS/SQS", "NumberOfMessagesDeleted", dims,
        "Sum", BASELINE_PERIOD_SECONDS, start_rate, end_rate
    )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        ts_hist, v_hist = get_metric_data_all(
            cw, namespace, metric_name, dimensions, stat, period, start_hist, end_hist
        )
        if not v_hist:
            raise RuntimeError(
        ...
        ts_cur, v_cur = get_metric_data_all(
            cw, namespace, metric_name, dimensions, stat, period, start_cur, end_cur
        )
